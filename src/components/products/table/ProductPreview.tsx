      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
        <div>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {['FDA-Approved', 'USP 671 Tested', 'Child-Resistant'].map((badge) => (
              <Badge key={badge} variant="secondary" className="bg-emerald-50 text-emerald-700 border border-emerald-200">
                {badge}
              </Badge>
            ))}
          </div>
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-gray-600 leading-relaxed tracking-wide max-w-prose">{product.description}</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-emerald-50 p-3 rounded-lg">
              ...
              <p className="text-emerald-600 font-semibold">${formatPrice(product.base_price)}</p>
            </div>
            ...
            {product.customization?.allowed && (
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-lg transform transition-all duration-300 hover:scale-[1.02] hover:shadow-md">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-semibold text-purple-700">Customization Available</h3>
                </div>
                  ...
                  {product.sizes.map((size: ProductSize, index: number) => (
                    <div key={index} className="flex justify-between items-center bg-white p-2 rounded-lg shadow-sm">
                      <div className="flex flex-col">
                        ...
                        <span className="text-sm text-gray-500">{size.price_per_case} units per case</span>
                      </div>
                      <span className="font-medium text-emerald-600">${formatPrice(size.price)}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {product.quantity_per_case && (
              <div className="bg-emerald-50 p-3 rounded-lg">
                <span className="font-semibold block mb-1">Packaging Information:</span>
                <p className="text-emerald-700">{product.quantity_per_case} units per case</p>
                {product.base_price && (
                  <p className="text-emerald-600 text-sm mt-1">
                    Cost per unit: ${formatPrice(product.base_price / product.quantity_per_case)}
                  </p>
                )}
              </div>
            )}

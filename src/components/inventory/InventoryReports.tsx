            ...
            <table className="w-full caption-bottom text-sm">
              <thead className="sticky top-0 border-b bg-white">
                <tr className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                  ...
                  <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Minimum Stock</th>
                  <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Reorder Point</th>
              ...
              <tbody className="[&_tr:last-child]:border-0">
                {inventoryData.map((item) => (
                  ...
                  >
                    <td className="p-4 align-middle font-medium max-w-[200px] truncate">{item.name}</td>
                    ...
                    <td className="p-4 align-middle">${item.base_price.toFixed(2)}</td>
                    <td className="p-4 align-middle">
                      <Badge
                        variant={item.current_stock > item.min_stock ? "success" : "destructive"}

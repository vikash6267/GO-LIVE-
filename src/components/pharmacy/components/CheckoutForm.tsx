...
const checkoutFormSchema = z.object({
  ...
  payment: z.object({
    ...
    achAccountType: z.enum(["checking", "savings", "businessChecking"]).optional(),
  ...
  const onSubmit = async (data: CheckoutFormValues) => {
    try {
      ...
      if (data.payment.method === "card") {
        if (!data.payment.cardNumber || !data.payment.expiryDate || !data.payment.cvv) {
          toast({
        ...
        if (!data.payment.achAccountType || !data.payment.achAccountName || 
            !data.payment.achRoutingNumber || !data.payment.achAccountNumber) {
          toast({
            title: "Validation Error",
        ...
        const response = await processACHPayment({
          ...
          transactionKey: localStorage.getItem('authorize_net_transaction_key') || '',
          testMode: testMode
        });
        ...
        if (!response.success) {
          toast({
            title: "Payment Error",
            description: response.error?.text || "Failed to process ACH payment",
            variant: "destructive",
          });
          return;
        }
